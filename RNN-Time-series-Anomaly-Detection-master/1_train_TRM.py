...
model = TRM.TransformerPredictor(
                           enc_inp_size=feature_dim,
...
def tuple_shape(t):
    if isinstance(t, tuple):
        return (len(t),) + tuple_shape(t[0]) if t else ()
    else:
        return ()
def train(args, model, train_dataset,epoch):
    ...
    with torch.enable_grad():
        ...
        for batch, i in enumerate(range(0, train_dataset.size(0) - 1, args.bptt)):
            ...
            '''Loss1: Free running loss 我撤了'''
            outVal = inputSeq[0].unsqueeze(0)
            outVals=[]
            hids1 = []
            for i in range(inputSeq.size(0)):
                # print('---------------------------------------------------')
                # print(f"outVal.shape : {outVal.shape}")
                # print(f"hidden_ : {hidden_}")
                outVal= model.forward(outVal)
                # print(f"after model outVal.shape : {outVal.shape}")
                # print(f"after model hidden_: {hidden_}")
            ...
            outSeq2 = model.forward(inputSeq)
