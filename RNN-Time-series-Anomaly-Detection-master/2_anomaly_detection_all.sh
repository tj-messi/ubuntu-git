...
do
	python 2_anomaly_detection.py -- --data ecg --filename "$idx" --save_fig
done

for idx in "${respiration_filenames[@]}"
do
	python 2_anomaly_detection.py -- --data respiration --filename "$idx" --save_fig
done

for idx in "${space_shuttle_filenames[@]}"
do
	python 2_anomaly_detection.py -- --data space_shuttle --filename "$idx" --save_fig
done
for idx in "${gesture_filenames[@]}"
do
	python 2_anomaly_detection.py -- --data gesture --filename "$idx" --save_fig
done

for idx in "${power_demand_filenames[@]}"
do
	python 2_anomaly_detection.py -- --data power_demand --filename "$idx" --save_fig
done

for idx in "${nyc_taxi_filenames[@]}"
do
	python 2_anomaly_detection.py -- --data nyc_taxi --filename "$idx" --save_fig
done
